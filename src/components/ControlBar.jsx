        ...
        <div style={{ display: 'flex', gap: '6px', flex: '0 1 auto', justifyContent: 'flex-end', minWidth: '220px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
            <button
              ...
              title={showRoute ? t.routeOff : t.routeOn}
              style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '6px', border: `1px solid ${styles.border}`, backgroundColor: showRoute ? styles.accent : 'transparent', color: showRoute ? '#fff' : styles.text, cursor: 'pointer' }}
            >
              {showRoute ? <Icons.Route /> : <Icons.RouteOff />}
            </button>
            <button
              onClick={onToggleMap}
              title={showMap ? t.mapOff : t.mapOn}
              style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '6px', border: `1px solid ${styles.border}`, backgroundColor: showMap ? styles.accent : 'transparent', color: showMap ? '#fff' : styles.text, cursor: 'pointer' }}
            >
      ...
      {mode === 'recommend' && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', width: '100%' }}>
          <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexShrink: 0 }}>
            ...
            <label
              style={{ fontSize: '13px', display: 'flex', alignItems: 'center', cursor: 'pointer', whiteSpace: 'nowrap', opacity: isPro ? 1 : 0.6 }}
              onClick={(e) => { if (!isPro) handleUpgradeClick(e); }}
            >
          ...
          {!useCurrentLoc && (
            <input
              ...
              style={{ ...miniInput, flex: '1 1 100px' }}
            />
          )}
